...
namespace Himesyo.Check
{
    ...
    public static class HashExtend
    {
        ...
        /// <returns></returns>
        public static uint ComputeCrc32(byte[] bytes, uint crc = 0xFFFFFFFF)
        {
        ...
        /// <param name="crc"></param>
        /// <returns></returns>
        public static uint ComputeCrc32(Stream stream, uint crc = 0xFFFFFFFF)
        {
            byte data;
    ...
    public class Crc
    {
        ...
        {
            int bit = (int)mode;
            ExceptionHelper.ThrowArgumentOut(bit <= 0, nameof(mode), mode, "算法类型必须大于 0 且小于等于 64 。");
            this.bit = bit;
            this.poly = poly;
